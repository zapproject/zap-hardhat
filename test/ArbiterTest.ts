...
const routeKeys = [1];
const params = [ethers.utils.sha256(ethers.utils.toUtf8Bytes("param1")), ethers.utils.sha256(ethers.utils.toUtf8Bytes("param2"))];
...
describe("ZapBondage", () => {
    ...
    it("ARBITER_1 - initiateSubscription() - Check subscription", async function () {
        ...
        await arbiter.connect(subscriber).initiateSubscription(oracle.address, specifier, params, publicKey, 10);
        console.log("finished")
        const res = await arbiter.getSubscription(oracle.address, subscriber.address, specifier);
    ...
    it("ARBITER_2 - initiateSubscription() - Check subscription block must be more than 0", async function () {
        ...
        await expect(arbiter.connect(subscriber).initiateSubscription(oracle, specifier, params, publicKey, 0, )).to.reverted
    });
    it("ARBITER_3 - initiateSubscription() - Check user cannot inititate subscription for same subscriber once", async function () {
        ...
        await expect(arbiter.connect(subscriber).initiateSubscription(oracle, specifier, params, publicKey, 10, )).to.reverted
    });
    ...
    it("ARBITER_4 - endSubscriptionProvider() - Check ending subscription", async function () {
        ...
        await arbiter.connect(oracle).endSubscriptionProvider(subscriber.address, specifier);
    ...
    it("ARBITER_5 - endSubscriptionProvider() - Check that user can't end uninitialized subscription", async function () {
        ...
        await expect(arbiter.connect(oracle).endSubscriptionProvider(subscriber.address, specifier)).to.reverted;
    ...
    it("ARBITER_6 - endSubscriptionSubscriber() - Check that user can't end uninitialized subscription", async function () {
        ...
        await expect(arbiter.connect(subscriber).endSubscriptionProvider(subscriber.address, specifier)).to.reverted;
    });
    ...
    it("ARBITER_7 - endSubscriptionSubscriber() - Check that only subscriber can end subscription by subscriber", async function () {
        ...
        await expect(arbiter.connect(subscriber).endSubscriptionSubscriber(oracle, specifier)).to.reverted;
    });
    ...
    it("ARBITER_10 - endSubscriptionProvider() - Check that subscriber receives any unused dots", async function () {
        ...
        let postEscrowBal = await bondage.getBoundDots(subscriber.address, oracle.address, specifier);
        ...
        await arbiter.connect(subscriber).endSubscriptionSubscriber(oracle.address, specifier);
        
        // 6 blocks have passed, and we include the first block in our calcuation, so we should receive 10-7=(3) dots back
        let postCancelBal = await bondage.getBoundDots(subscriber.address, oracle.address, specifier);
        expect(postCancelBal.toString()).to.be.equal("93");

        // check that the provider received their 7 dots
        let postCancelProviderBal = await bondage.getBoundDots(oracle.address, oracle.address, specifier);
