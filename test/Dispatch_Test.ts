    ...
    beforeEach( async()=>{
      console.log(getEventHashSigs())
        ...
        //  console.log("deployed")
         ...
         dispatch= await (await dispatchFactory.deploy(coordinator.address)) as Dispatch;
   ...
   async function prepareProvider( account = owner, curveParams = piecewiseFunction) {
    ...
    let IncomingInterface = new ethers.utils.Interface(abi)
        it("DISPATCH_1 - respond1() - Check that we can make a simple query and the correct events are emitted", async function () {
          ...
          console.log((await bondage.getBoundDots(subscriber.address,owner.address,spec1)).toString())
          let result= await subscriber.connect(subscriberAccount).testQuery(owner.address, query, spec1, params);
          let r=await result.wait()
            
          let expected=["Escrowed(address,address,bytes32,uint256)","Incoming(uint256,address,address,string,bytes32,bytes32[],bool)","MadeQuery(address,string,uint256)"]
